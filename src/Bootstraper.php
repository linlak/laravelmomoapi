...
class Bootstraper
{
	...
	private function setMomo(MomoApp $momo, ApiUser $apiUser)
	{
		...
		if ("" !== (string) $apiUser->api_key) {
			...
			if (
				($apiUser->accessToken === null) || ($apiUser->AccessToken !== null && (string) $apiUser->AccessToken->access_token === "") || ((string) $apiUser->AccessToken->expires_at === "") || ((string) $apiUser->AccessToken->expires_at !== "" && (new Carbon($apiUser->accessToken->expires_at))->diffInSeconds($apiUser->accessToken->freshTimestamp()) <= 500)
			) {
	...
	public function saveRequestToPay(RequestToPayResponse $result, Model $pay, $api_primary, $api_secondary)
	{
		if ($apiUser = $this->checkApiUser($api_primary, $api_secondary)) {
			$pytToDb = null;
			if ($apiUser->product) {
				switch ($apiUser->product) {
					case 'Collection':
						...
						$pytToDb->collectionable_id = $pay->id;
						$pytToDb->collectionable_type = $pytToDb->getActualClassNameForMorph(get_class($pay));
						break;
					case 'Disbursement':
						$pytToDb = new Disbursement;
						$pytToDb->disbursable_id = $pay->id;
						$pytToDb->disbursable_type = $pytToDb->getActualClassNameForMorph(get_class($pay));
						break;
					case 'Remittance':
						$pytToDb = new Remittance;
						$pytToDb->remittable_id = $pay->id;
						$pytToDb->remittable_type = $pytToDb->getActualClassNameForMorph(get_class($pay));
						break;
				}
				...
				$pytToDb->save();
				$pytToDb->refresh();
				if ($pytToDb instanceof Collection) {
					//broadcast event
				}

				if ($pytToDb instanceof Remittance) {
					//broadcast event
				}

				if ($pytToDb instanceof Disbursement) {
					//broadcast
				}
			}
		}
	...
	public function updateRequestToPay(RequestStatus $result, Model $pyt)
	{
		if ($result->resourceExists()) {
			...
			$pyt->save();
			$pyt->refresh();
			if ($pyt instanceof Collection) {
				//broadcast event
			}

			if ($pyt instanceof Remittance) {
				//broadcast event
			}

			if ($pyt instanceof Disbursement) {
				//broadcast
			}
		}
	}
	...
	public function registerCollectionsHook(Request $request)
	{
		...
		$momoHook->checkStatus($callBackData);
		return \response(200);
	}
